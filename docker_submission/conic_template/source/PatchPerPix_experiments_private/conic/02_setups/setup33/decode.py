...
def decode_sample(decoder, sample, **kwargs):
    ...
    logger.info("%s: processing %i batches, tta? %s", os.path.basename(sample), num_batches,
    ...
    for b in range(0, len(fg_coords), batch_size):
        ...
        if kwargs.get('test_time_aug'):
            pred_code_batchedT = [pred_code[fg_coord] for fg_coord in fg_coords_batched]
            pred_code_batched = []
            for pcb in pred_code_batchedT:
                pcb = np.hsplit(pcb.reshape((1, 4*code_units)), 4)
                pred_code_batched.extend(pcb)
        else:
            pred_code_batched = [pred_code[fg_coord].reshape((1, code_units))
                                 for fg_coord in fg_coords_batched]
        if kwargs.get('test_time_aug'):
            if len(pred_code_batched) < 4*batch_size:
                pred_code_batched = pred_code_batched + ([np.zeros(
                    (1, code_units))] * (4*batch_size - len(pred_code_batched)))
        else:
        ...
        for idx, fg_coord in enumerate(fg_coords_batched):
            if kwargs.get('test_time_aug'):
                ...
                prediction = predictions[4*idx+0]['affinities']
                # prediction = np.reshape(prediction, )
                prediction += np.flip(predictions[4*idx+1]['affinities'], 1)
                prediction += np.flip(predictions[4*idx+1]['affinities'], 2)
                prediction += np.rot90(predictions[4*idx+1]['affinities'], -1,  axes=(1,2))
                prediction = {'affinities': prediction/4}
            else:
...
def decode(**kwargs):
    ...
    if kwargs['mode'] == tf.estimator.ModeKeys.PREDICT:
        ...
        for idx, sample in enumerate(kwargs['samples']):
            print("decoding {}/{}: {}".format(idx, len(kwargs['samples']), sample))
            # decode each sample
            ...
            if "zarr" in kwargs['output_format']:
                # pred_fg = np.array(zarr.open(sample, 'r')[kwargs['fg_key']])
                pred_class = np.array(zarr.open(sample, 'r')[
                    kwargs.get('class_key', "volumes/pred_class")])
            else:
                with h5py.File(sample, 'r') as f:
                    # pred_fg = np.array(f[kwargs['fg_key']])
                    pred_class = np.array(f[kwargs.get('class_key', "volumes/pred_class")])
            ...
            #     compression='gzip'
            # )
            pred_labels = np.argmax(pred_class, axis=0)
            outf.create_dataset(
                "volumes/pred_labels",
                data=pred_labels,
                compression='gzip'
            ...
            # pred_labels2 = np.argmax(pred_class, axis=0)
            # outf.create_dataset(
            #     "volumes/pred_labels2",
            #     data=pred_labels2,
            #     compression='gzip'
            # )
