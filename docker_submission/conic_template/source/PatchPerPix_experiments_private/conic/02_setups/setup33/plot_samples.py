...
def get_pq(true, pred, match_iou=0.5, remap=True):
    ...
    return (
        ...
        [tp, fp, fn],
        paired_iou.sum(),
        [paired_true, paired_pred, unpaired_pred, unpaired_true],
        [pred,true],
        pairwise_iou
    )
...
def get_multi_pq_info(true, pred, nr_classes=6, match_iou=0.5):
    ...
    for idx in range(nr_classes):
        ...
        [DQ, SQ, PQ],[tp,fp,fn],paired_iou,[paired_true, paired_pred, unpaired_pred, unpaired_true],[_,_],pairwise_iou = get_pq(true_inst_oneclass, pred_inst_oneclass, remap=False)

        pq_oneclass_stats = [
            PQ,
            [paired_true, paired_pred, unpaired_pred, unpaired_true], # indizes
...
def main():
    fls = os.listdir("/home/peter/data/datasets/data_conic/lizard_zarr/val/patches")
    ...
    pred_class_list = []
    for fn in fls:
        zfl = zarr.open(os.path.join(
            "/home/peter/data/datasets/data_conic/lizard_zarr/val/patches", fn), 'r')
        raw_list.append(np.array(zfl['volumes/raw']))
        gt_inst_list.append(np.array(zfl['volumes/gt_instances']))
        gt_class_list.append(np.array(zfl['volumes/gt_class']))


        fn = os.path.splitext(fn)[0] + ".hdf"
        with h5py.File(os.path.join(sys.argv[1], fn), 'r') as hfl:
            pred_inst_list.append(np.array(hfl['vote_instances']))

        with h5py.File(os.path.join(sys.argv[2], fn), 'r') as hfl:
            pred_class_list.append(np.array(hfl['volumes/pred_labels']))
    ...
    for idx in range(len(fls)):
        # fl = os.path.splitext(fl)[0] + ".hdf"
        # preds_classT.append(preds_class[flsT.index(fl)])
        pred_classL_list.append(preds_class[idx])


    print(type(gt_inst_list), type(gt_class_list), type(pred_inst_list), type(pred_class_list), type(raw_list))

    parallel = 48
    if parallel > 1:
        Parallel(n_jobs=parallel, verbose=1) \
            (delayed(work)(gt_inst, gt_class, pred_inst, pred_classL, pred_class, raw) for gt_inst, gt_class, pred_inst, pred_classL, pred_class, raw  in zip(gt_inst_list, gt_class_list, pred_inst_list, pred_classL_list, pred_class_list, raw_list))
    else:
        for gt_inst, gt_class, pred_inst, pred_classL, pred_class, raw  in zip(gt_inst_list, gt_class_list, pred_inst_list, pred_classL_list, pred_class_list, raw_list):
            work(gt_inst, gt_class, pred_inst, pred_classL, pred_class, raw)


def work(gt_inst, gt_class, pred_inst, pred_classL, pred_class, raw):
    title_size = 16
    print(gt_inst.shape, gt_class.shape, pred_inst.shape, pred_class.shape, raw.shape)
    raw = np.moveaxis(raw, 0, -1)
    gt_inst = np.squeeze(gt_inst, 0)
    gt_class = np.squeeze(gt_class, 0)
    pred_inst = np.squeeze(pred_inst, 0)
    pred_classL = np.argmax(pred_classL, axis=0)
    print(gt_inst.shape, gt_class.shape, pred_inst.shape, pred_class.shape, pred_classL.shape, raw.shape)


    gt_inst = remap_label(gt_inst)
    pred_inst = remap_label(pred_inst)

    fig, ax = plt.subplots(3,8, figsize=(44,12))
    ax[0,0].imshow(raw)
    ax[1,0].imshow(plot_instances(gt_inst), interpolation='nearest')
    ax[0,1].imshow(plot_instances(gt_class*5), interpolation='nearest')
    ax[1,1].imshow(plot_instances(pred_classL*5))
    ax[2,0].imshow(plot_instances(pred_inst), interpolation='nearest')
    ax[2,1].imshow(plot_instances(pred_class*5))
    ...
    for i in np.unique(pred_inst):
        if i == 0:
            continue
        ids, cnts = np.unique(pred_classL[pred_inst==i], return_counts=True)
        if ids[0] == 0:
            ids = ids[1:]
            cnts = cnts[1:]
        if len(ids) == 0:
    ...
    ax[1,1].title.set_text('pred_ct lorenz')
    ax[2,0].title.set_text('pred_instances')
    ax[2,1].title.set_text('pred_ct peter')
    ...
    gt = np.stack([gt_inst, gt_class], axis=-1)
    pred = np.stack([pred_inst, pred_classL], axis=-1)
    metr = get_multi_pq_info(true = gt, pred = pred)
    x = 2
    PQ_l = []
    for idx, ([PQ, [tp_true_idx, tp_pred_idx, fp_idx, fn_idx], [true_inst_oneclass, pred_inst_oneclass]]) in enumerate(metr[0]):
        gt_tmp = true_inst_oneclass
        pred_tmp = pred_inst_oneclass
        # pred_tmp = pred[idx, ...,0]
        tmp = np.zeros([256,256,3])
        # for idx in tp_true_idx:
        #     tmp[gt_tmp==idx,1]=1 # green tp
        for idx in tp_pred_idx:
            tmp[pred_tmp==idx,1]=1 # green tp
        for idx in fp_idx:
    ...
    for idx, ([PQ, [tp_true_idx, tp_pred_idx, fp_idx, fn_idx], [true_inst_oneclass, pred_inst_oneclass]]) in enumerate(metr[0]):
        ...
        for idx in fn_idx:
            tmp[gt_tmp==idx,2]=1  # blue fn
        ax[2,x].imshow(tmp)
        ax[2,x].title.set_text('TP:green, FP: red, FN: blue, PQ: '+str(PQ)[:5])
        ax[2,x].title.set_size(title_size)
        x += 1
        # PQ_l.append(PQ)
    ...
    ax[0,1].title.set_size(title_size)
    plt.tight_layout()
    plt.savefig('test_samples2/mPQ_'+str(PQ)[:6]+'.png', facecolor='white')
    # plt.show()
    # exit()
