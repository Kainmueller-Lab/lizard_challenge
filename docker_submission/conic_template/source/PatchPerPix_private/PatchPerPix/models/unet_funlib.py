...
def conv_pass(
    ...
    if shortcut:
        fmaps_in = crop_spatial(fmaps_in, out_shape)
...
def crop_to_factor(fmaps_in, factor, kernel_sizes):
    ...
    print("spatial shapes", target_spatial_shape, spatial_shape, convolution_crop,
          target_spatial_shape!=tuple(spatial_shape))
...
def unet(
    ...
    if kernel_size_down is None:
        kernel_size_down = [[kernel_size, kernel_size]]*(len(downsample_factors) + 1)
    if kernel_size_up is None:
        kernel_size_up = [[kernel_size, kernel_size]]*len(downsample_factors)

    if constant_upsample is None:
